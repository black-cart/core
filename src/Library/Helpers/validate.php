...
if (!function_exists('bc_check_view')) {
    /**
     * Check view exist
     *
     ...
     */
    function bc_check_view($view)
    {
        if(!view()->exists($view)) {
            bc_report('View not found '.$view);
...
if (!function_exists('bc_clean')) {
    /**
     * Clear data
     */
    function bc_clean($data = null, $exclude = [], $level_hight = null)
    {
        if ($level_hight) {
            if (is_array($data)) {
        ...
        if (is_array($data)) {
            array_walk($data, function (&$v, $k) use ($exclude, $level_hight) {
                if (is_array($v)) {
                    $v = bc_clean($v, $exclude, $level_hight);
                } else {
