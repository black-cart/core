...
if (!function_exists('bc_clear_cache')) {
    /**
     * Clear cache
     *
     * @param   [string]  $domain
     ...
     */
    function bc_clear_cache($typeCache = 'cache_all', $storeId = null)
    {
        try {
...
if (!function_exists('bc_set_cache')) {
/**
 * [bc_set_cache description]
 *
 ...
 */
    function bc_set_cache($cacheIndex , $value, $time = null)
    {
        if(empty($cacheIndex)) {
            return ;
        }
        $seconds = $time ?? (bc_config_global('cache_time') ?? 600);
