...
namespace BlackCart\Core\Library\ShoppingCart;

use BlackCart\Core\Library\ShoppingCart\Exceptions\CartAlreadyStoredException;
use BlackCart\Core\Library\ShoppingCart\Exceptions\UnknownModelException;
use Closure;
...
class Cart
{
     ...
     * @return \BlackCart\Core\Library\ShoppingCart\Cart
     */
     ...
     * @param float     $price
     * @param array     $options
     * @return \BlackCart\Core\Library\ShoppingCart\CartItem
     */
    ...
    public function total()
    {
        ...
        $total = $content->reduce(function ($total, CartItem $cartItem) {
            return $total + ($cartItem->qty * bc_tax_price($cartItem->price, $cartItem->tax));
        }, 0);
        return $total;
    }
    ...
    {
        return BC_DB_PREFIX.'shop_shoppingcart';
    }
    ...
    */
    public static function getListCart($instance = self::DEFAULT_INSTANCE)
    {
        ...
        $arrCart['subtotal'] = bc_currency_render($cart->subtotal());
        $arrCart['items'] = [];
        if ($cart->count()) {
            foreach ($cart->content() as $key => $item) {
                $product = \BlackCart\Core\Front\Models\ShopProduct::find($item->id);
                if ($product) {
                    $arrCart['items'][] = [
                        'id'        => $item->id,
                        'rowId'     => $item->rowId,
