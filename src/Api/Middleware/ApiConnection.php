...
namespace BlackCart\Core\Api\Middleware;
...
class ApiConnection
{
    ...
    {
        if(!bc_config('api_connection_required')) {
            return $next($request);
        }
        ...
        $check = \BlackCart\Core\Front\Models\ShopApiConnection::check($apiconnection, $apikey);
        if($check) {
