...
class ShopCustomer extends Authenticatable
{
     ...
     * @var array
     */
    protected $table = BC_DB_PREFIX.'shop_customer';
    protected $guarded = [];
    ...
    public function sendPasswordResetNotification($token)
    {
        ...
        if ($checkContent) {
            ...
            $url = bc_route('password.verify', ['token' => $token]);
            $dataReplace = [
            ...
            bc_send_mail('templates.' . bc_store('template') . '.mail.forgot_password', $dataView, $config, $dataAtt = []);
        }
    ...
    {
        $dataClean = bc_clean($dataInsert, 'password');
        $dataAddress = [
    ...
    public function hasVerifiedEmail() {
        return !$this->isVerified() && bc_config('customer_verify');
    }
    /**
    ...
    public function sendEmailVerify() {
        ...
        if ($this->hasVerifiedEmail()) {
            ...
            if ($checkContent) {
                ...
                bc_send_mail('templates.' . bc_store('template') . '.mail.customer_verify', $dataView, $config, $dataAtt = []);
                return true;
            }
        } 
        return false;
