...
Route::group(
    [
        'middleware' => BC_FRONT_MIDDLEWARE,
    ], 
    function () {
...
Route::middleware(BC_FRONT_MIDDLEWARE)
...
Route::group(
    ...
    function () use ($suffix, $langUrl) {
        ...
        //Route shop
        $prefixShop = bc_config('PREFIX_SHOP') ?? 'shop';
        Route::get($langUrl.$prefixShop, 'ShopProductController@allProductsProcessFront')
            ->name('shop');
        ...
        Route::get('locale/{code}', function ($code) {
            ...
            if (bc_route('home.lang') === redirect()->back()->getTargetUrl()
            ) {
                return redirect(bc_route('home.lang', ['lang' => $code]));
            }
        ...
        // subdomain handle process image
        Route::group(['domain' => '{subdomain}.'.env('APP_URL')], function()
        {
            Route::get('/handle/{w?}/{f?}/{url?}', "CloundImageController@handle");
        });
    }
);