...
class PermissionController extends RootAdminController
{
    ...
    public function __construct()
    {
        ...
        foreach ($routes as $route) {
            if (Str::startsWith($route->uri(), BC_ADMIN_PREFIX)) {
                $prefix = BC_ADMIN_PREFIX?$route->getPrefix():ltrim($route->getPrefix(),'/');
                $routeAdmin[$prefix] = [
                    'uri'    => 'ANY::' . $prefix . '/*',
                    'name'   => $prefix . '/*',
                    'method' => 'ANY',
    ...
    public function index()
    {
        $data = [
            ...
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => bc_route_admin('admin_permission.delete'),
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $dataTr[] = [
                ...
                'action' => '
                    <a href="' . bc_route_admin('admin_permission.edit', ['id' => $row['id']]) . '"><span title="' . trans('permission.admin.edit') . '" type="button" class="btn btn-flat btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
...
//menuRight
        $data['menuRight'][] = '<a href="' . bc_route_admin('admin_permission.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
...
//menuSort
        ...
        $data['urlSort'] = bc_route_admin('admin_permission.index', request()->except(['_token', '_pjax', 'sort_order']));
 ...
 */
    public function create()
    {
        $data = [
            ...
            'url_action' => bc_route_admin('admin_permission.create'),
 ...
 */
    public function edit($id)
    {
        ...
        $data = [
            ...
            'routeAdmin' => $this->routeAdmin,
            'url_action' => bc_route_admin('admin_permission.edit', ['id' => $permission['id']]),
        ];
        return view($this->templatePathAdmin.'Auth.permission')
            ->with($data);
    }
 ...
 */
    ...
    {
        $prefix = BC_ADMIN_PREFIX?BC_ADMIN_PREFIX.'/':'';
        return [
            $prefix . 'login',
            $prefix . 'logout',
            $prefix . 'forgot',
