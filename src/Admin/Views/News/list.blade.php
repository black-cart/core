...
<div class="col-md-12">
  <div class="card" >
    <div class="card-header">
        <h4 class="card-title m-0">Tìm kiếm Blog/News</h4>
    </div>
    <div class="card-body" >
      <form action="{{bc_route_admin('admin_news.index')}}" id="button_search">
        <div class="row justify-content-between">
            ...
            <div class="col-sm-7 d-flex justify-content-end">
                <div class="form-group d-flex">
                  <input type="text" name="keyword" class="form-control mr-3" value="{{$keyword}}" placeholder="{{ trans('news.admin.search_place') }}">
        ...
        <div class="tools float-right">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end">
                    <a class="dropdown-item" id="button_create_new" href="{{bc_route_admin('admin_news.create')}}">{{trans('admin.add_new')}}</a>
            ...
            <thead>
              <tr>
                ...
                @endif
                  ...
                  <th class="text-center">{{trans('news.sort')}}</th>
                  <th class="text-center">{{trans('news.status')}}</th>
                  <th>{{trans('order.admin.action')}}</th>
               </tr>
            </thead>
            <tbody>
              @foreach ($dataNews as $keyRow => $row)
              <tr>
                  @if (!empty($removeList))
                  ...
                  <td>{{$row['id']}}</td>
                  <td>{{ $row['title'] }}</td>
                  <td>{!! bc_image_render($row['image'], '100px',null, $row['title']) !!}</td>
                  ...
                  <td>
                    @include($templatePathAdmin.'Component.action_list',['url_edit'=> bc_route_admin('admin_news.edit',['id'=>$row['id']]),'id'=>$row['id']])
                  </td>
              </tr>
              @endforeach
            </tbody>
