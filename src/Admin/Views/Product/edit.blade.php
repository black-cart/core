...
@section('main')
<!-- form start -->
<form action="{{ bc_route_admin('admin_product.edit',['id'=>$product['id']]) }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main" 
...
<div class="row">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between pb-3 align-content-center">
                @if (bc_config_admin('product_kind'))
                    <h4 class="card-title align-self-center mb-0">{{$descriptions['en']['name']}} ({!! $kinds[$product->kind]??'' !!})</h4>
                @endif
            ...
            <div class="card card-{{ ($product->kind == 1) ? 'success': (($product->kind == 2) ? 'danger': '') }}">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group {{ $errors->has('descriptions.'.$code.'.keyword') ? ' text-red' : '' }}">
                        <label for="{{ $code }}__keyword">{{ trans('product.keyword') }}</label>
                    ...
                    @if($product->kind == BC_PRODUCT_SINGLE || $product->kind == BC_PRODUCT_BUILD)
            ...
            <div class="card-body">
                ...
                <div class="form-group kind  {{ $errors->has('image') ? ' text-red' : '' }}">
                    ...
                    <div class="row" id="toolTipGenerator">
                        ...
                        @if($product->kind == BC_PRODUCT_BUILD)
                            @if (old('hotSpots',$product->builds))
                                <input id="old_hotSpots" type="hidden" value="{{ json_encode(old('hotSpots',$product->builds)) }}">
                            @endif
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="hidden" id="image" name="image" value="{!! $image !!}" class="image"/>
                            </div>
                            <div class="col-md-12">
                        ...
                        @else
                            <div class="col-md-6">
                                <button class="btn btn-primary mb-2 lfm" type="button" data-kind="{{$product->kind}}" 
                                    data-input="image" data-preview="preview_image" data-type="product">
                                    {{ trans('product.admin.choose_image') }} <i class="tim-icons icon-cloud-upload-94"></i>
                                </button>
                                <div class="thumbnail">
                                    <div class="img_holder d-flex" id="preview_image" >
                                        @if($image)
                                            <img src="{{ asset($image) }}">
                                        @endif
                                    </div>
                ...
                @if($product->kind == BC_PRODUCT_SINGLE)
                    <div class="form-group kind  {{ $errors->has('sub_image') ? ' text-red' : '' }}">
                        @php
                            $old_sub_image = $product->images->first();
                            if ($old_sub_image) {
                                $old_sub_image = $old_sub_image->toArray();
                            }else{
                                $old_sub_image['image'] = '';
                                $old_sub_image['type_show'] = '';
                            }
                            $listsubImages = old('sub_image',$old_sub_image['image']);
                            $sub_images = explode(',',$listsubImages);
                            $type_show_image_desc = old('type_show_image_desc',$old_sub_image['type_show']);
                        @endphp
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="form-check form-check-radio mb-2">
                                    <label class="form-check-label" for="radioPrimarydefault">
                                        ...
                                        value="default" {{ ($type_show_image_desc == 'default' || $type_show_image_desc == '') ? "checked" :'' }} >
                                        <span class="form-check-sign"></span>
                                        Default
                                    </label>
                                    <label class="form-check-label" for="radioPrimarythreesixty">
                                        ...
                                        value="threesixty" {{ ($type_show_image_desc == 'threesixty')?'checked':'' }}>
                                        <span class="form-check-sign"></span>
                                        360 degree
                                    </label>
                                </div>
                                <div class="thumbnail">
                                    ...
                                    <input type="hidden" id="sub_image" name="sub_image" value="{!! $listsubImages !!}" class="sub_image" />
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-group kind  {{ $errors->has('sku') ? ' text-red' : '' }}">
                    ...
                    <input type="text" id="sku" name="sku" placeholder="{{ trans('product.sku_validate') }}" 
                ...
                <div class="form-group kind  {{ $errors->has('alias') ? ' text-red' : '' }}">
                    ...
                    <input type="text"  id="alias" name="alias" placeholder="{{ trans('product.alias_validate') }}" 
                        value="{!! old('alias', $product->alias) !!}" class="form-control alias" />
                    @include($templatePathAdmin.'Component.feedback',['field'=>'alias'])
                </div>
                @if($product->kind == BC_PRODUCT_SINGLE || $product->kind == BC_PRODUCT_BUILD)
                    @if (bc_config_admin('product_brand') && ($product->kind == BC_PRODUCT_SINGLE || $product->kind == BC_PRODUCT_BUILD))
                        {{-- select brand --}}
                        <div class="form-group kind kind0 kind1 {{ $errors->has('brand_id') ? ' text-red' : '' }}">
                            ...
                            <select class="form-control brand_id selectpicker" name="brand_id">
                                ...
                                @foreach ($brands as $k => $v)
                                    <option value="{{ $k }}" {{ (old('brand_id') ==$k || (!old() && $product->brand_id ==$k) ) ? 'selected':'' }}>{{ $v->name }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    @if (bc_config_admin('product_supplier') && ($product->kind == BC_PRODUCT_SINGLE || $product->kind == BC_PRODUCT_BUILD))
                        {{-- select supplier --}}
                        <div class="form-group kind kind0 kind1 {{ $errors->has('supplier_id') ? ' text-red' : '' }}">
                            ...
                            <select class="form-control supplier_id selectpicker"
                                ...
                                <option value="{{ $k }}" {{ (old('supplier_id') ==$k || (!old() && $product->supplier_id ==$k) ) ? 'selected':'' }}>{{ $v->name }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    @if (bc_config_admin('product_cost'))
                        {{-- cost --}}
                        <div class="form-group kind kind0 kind1 {{ $errors->has('cost') ? ' text-red' : '' }}">
                            <label for="cost">{{ trans('product.cost') }}</label>
                            <input type="number" id="cost" name="cost" value="{!! old('cost',$product->cost) !!}" class="form-control cost" />
                    ...
                    @if (bc_config_admin('product_price'))
                        ...
                        <div class="form-group kind {{ $errors->has('price') ? ' text-red' : '' }}">
                            ...
                            <input type="number" id="price" name="price" value="{!! old('price',$product->price) !!}" class="form-control price" placeholder="" />
                    ...
                    @if (bc_config_admin('product_tax'))
                        ...
                        <div class="form-group kind {{ $errors->has('tax_id') ? ' text-red' : '' }}">
                            ...
                            <select class="form-control tax_id selectpicker" name="tax_id">
                                ...
                                @foreach ($taxs as $k => $v)
                                    <option value="{{ $k }}" {{ (old('tax_id') ==$k || (!old() && $product->tax_id ==$k) ) ? 'selected':'' }}>{{ $v->name }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    @if (bc_config_admin('product_promotion'))
                        {{-- price promotion --}}
                        <div class="form-group kind kind0 kind1 {{ $errors->has('price_promotion') ? ' text-red' : '' }}">
                            <div class="mb-3 justify-content-between d-flex align-content-center">
                            ...
                            <div class="row pl-3 pr-3">
                                @if (old('price_promotion') || $product->promotionPrice)
                                    <div class="price_promotion col-md-12">
                                        <div class="form-group">
                                            <input type="number" id="price_promotion" name="price_promotion" 
                                            value="{!! old('price_promotion',$product->promotionPrice->price_promotion ?? '') !!}" class="form-control price"/>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="price_promotion_start" autocomplete="false" 
                                                    ...
                                                    value="{!!old('price_promotion_start', bc_datetime_to_date($product->promotionPrice->date_start))!!}"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="price_promotion_end" autocomplete="false" 
                                                    ...
                                                    value="{!!old('price_promotion_end', bc_datetime_to_date($product->promotionPrice->date_end))!!}"
                    ...
                    {{-- minimum --}}
                    @if (($product->kind == BC_PRODUCT_SINGLE || $product->kind == BC_PRODUCT_BUILD))
                        <div class="form-group kind kind0 kind1{{ $errors->has('minimum') ? ' text-red' : '' }}">
                            <label for="minimum">{{ trans('product.minimum') }}</label>
                            <input type="number" id="minimum" name="minimum" value="{!! old('minimum',$product['minimum']) !!}" class="form-control minimum" placeholder="{{ trans('product.minimum_help') }}" />
                    ...
                    @if (bc_config_admin('product_available'))
                        {{-- date available --}}
                        <div class="form-group kind kind0 kind1 {{ $errors->has('date_available') ? ' text-red' : '' }}">
                    ...
                    @if (bc_config_admin('product_property'))
                        @if ($product->kind == BC_PRODUCT_SINGLE)
                        {{-- property --}}
                        <div class="form-group kind kind0 kind1 {{ $errors->has('property') ? ' text-red' : '' }}">
                            <label for="property"><a target=_new href="{{ bc_route_admin('admin_product_property.index') }}">{{ trans('product.property') }}</a></label>
                            <input type="text" name="download_path" value="{{ old('download_path', $product->downloadPath->path ?? '') }}" class="form-control" id="download_path"
                                placeholder="{{ trans('product.properties.download_path') }}" 
                                style="{{ (old('property', $product->property) != BC_PROPERTY_DOWNLOAD) ? 'display:none':'' }}"  />
                            ...
                            <div class="form-group kind {{ $errors->has('fields.'.$field->code) ? ' text-red' : '' }}">
                                ...
                                @elseif($field->option == 'select')
                                    @if ($default)
                                        ...
                                        <option value=""></option>
                                            @foreach ($default as $key => $name)
                                                <option value="{{ $key }}" {{ (old('fields.'.$field->code, ($fields[$field->code]['text'] ?? '')) == $key) ? 'selected':'' }}>
                                                    {{ $name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                @else
                                    ...
                                    class="form-control {{ $field->code }}" placeholder="" >{{ old('fields.'.$field->code, ($fields[$field->code]['text'] ?? '')) }}
                                    </textarea>
                                @endif
    ...
    <div class="col-md-4">
                    ...
                    @php
                        ...
                        foreach ($getDataAtt as $groupId => $row) {
                            foreach ($row as $key => $value) {
                                ...
                                $arrAtt[$groupId]['code'][] = $value['code'];
                                $arrAtt[$groupId]['images'][] = $value['images'];
                                $arrAtt[$groupId]['type_show'][] = $value['type_show'];
                            }
                        }
                                        ...
                                        <div class="col-md-7 text-center">
                                            ...
                                            <div class="form-check form-check-radio mb-2">
                                                <label class="form-check-label" for="radioPrimarydefault_{{$key}}">
                                                    <input class="form-check-input" type="radio" name="attribute[{{$attGroupId}}][type_show][{{$key}}]" 
                                                    id="radioPrimarydefault_{{$key}}" value="default" {{ ($dataAtt[$attGroupId]['type_show'][$key] == 'default')?'checked':'' }} >
                                                    <span class="form-check-sign"></span>
                                                    Default 
                                                </label>
                                                <label class="form-check-label" for="radioPrimarythreesixty_{{$key}}">
                                                    <input class="form-check-input" type="radio" name="attribute[{{$attGroupId}}][type_show][{{$key}}]" 
                                                    id="radioPrimarythreesixty_{{$key}}" value="threesixty" {{ ($dataAtt[$attGroupId]['type_show'][$key] == 'threesixty')?'checked':'' }}>
                                                    <span class="form-check-sign"></span>
                                                    360 degree
                                                </label>
                                            </div>
            ...
            <div class="card-body">
                @if (bc_config_admin('product_weight'))
                    ...
                    <div class="form-group kind {{ $errors->has('weight') ? ' text-red' : '' }}">
                        ...
                        <input type="number" id="weight" name="weight" value="{!! old('weight',$product->weight) !!}" class="form-control weight" placeholder="" />
                ...
                @if (bc_config_admin('product_length'))
                    ...
                    <div class="form-group kind {{ $errors->has('length') ? ' text-red' : '' }}">
                        <label for="length">{{ trans('product.length') }}</label>
                            <input type="number" id="length" name="length" value="{!! old('length',$product->length) !!}" class="form-control length" placeholder="" />
                    ...
                    <div class="form-group kind   {{ $errors->has('height') ? ' text-red' : '' }}">
                        ...
                        <input type="number" id="height" name="height" value="{!! old('height',$product->height) !!}" class="form-control height" placeholder="" />
                    ...
                    <div class="form-group kind {{ $errors->has('width') ? ' text-red' : '' }}">
                        ...
                        <input type="number" id="width" name="width" value="{!! old('width',$product->width) !!}" class="form-control width" placeholder="" />
        ...
        <div class="card">
            ...
            <div class="card-body kind0 kind1 kind2">
                {{-- sort --}}
                    <div class="form-group {{ $errors->has('sort') ? ' text-red' : '' }}">
                        <label for="sort">{{ trans('product.sort') }}</label>
                        <input type="number" id="sort" name="sort" value="{!! old('sort',$product['sort']) !!}" class="form-control sort" placeholder="" />
                ...
                {{-- status --}}
                    <div class="form-group">
                        ...
                        <input type="checkbox" {{ old('status',$product['status'])?'checked':''}} name="status" class="bootstrap-switch" data-on-label="ON" data-off-label="OFF">
                    </div>
                {{-- //status --}}
            </div>
    ...
    if (picker) {
        ...
        htmlPickColorThef = '<div class="col-md-7">'+
                                    ...
                                    '<div class="thumbnail mb-2">'+
                                        '<div class="img_holder color-thief" data-group="'+key+'" id="preview_attribute_image_'+key+'">'+
                                        '</div>'+
...
function ChangeContentBuildProduct($this) {
    var val = $($this).val();
    $('.content_type textarea').val('');
    $('.content_type textarea').trigger('keyup');
    $('.content_type').hide();
