      ...
      <div class="card card-primary card-outline card-outline-tabs">
        <div class="card-header p-0 border-bottom-0">
          <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
            ...
            <li class="nav-item">
              <a class="nav-link" href="{{ bc_route_admin('admin_plugin_online', ['code' => strtolower($code)]) }}" >{{ trans('plugin.online') }}</a>
            </li>
            @endif
            <li class="nav-item">
          ...
          <div class="tab-content" id="custom-tabs-four-tabContent">
            ...
            <table class="table table-hover text-nowrap table-bordered">
              ...
              <tbody>
                ...
                @endphp
                  <tr>
                    <td>{!! bc_image_render($pluginClass->image,'50px', '', $pluginClass->title) !!}</td>
                    <td>{{ $codePlugin }}</td>
                  ...
                  //End try cacth
                      ...
                      bc_report(json_encode($pluginClassName)." : ".$e->getMessage());
...
<script type="text/javascript">
  function enablePlugin(obj,key) {
      ...
      $.ajax({
        type: 'POST',
        dataType:'json',
        url: '{{ bc_route_admin('admin_plugin.enable') }}',
        data: {
          "_token": "{{ csrf_token() }}",
          "key":key,
          "code":"{{ $code }}"
  ...
  function disablePlugin(obj,key) {
      ...
      $.ajax({
        type: 'POST',
        dataType:'json',
        url: '{{ bc_route_admin('admin_plugin.disable') }}',
        data: {
          "_token": "{{ csrf_token() }}",
          "key":key,
          "code":"{{ $code }}"
  ...
  function installPlugin(obj,key) {
      ...
      $.ajax({
        type: 'POST',
        dataType:'json',
        url: '{{ bc_route_admin('admin_plugin.install') }}',
        data: {
          "_token": "{{ csrf_token() }}",
          "key":key,
          "code":"{{ $code }}"
      ...
      Swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
              type: 'POST',
              dataType:'json',
              url: '{{ bc_route_admin('admin_plugin.uninstall') }}',
              data: {
                "_token": "{{ csrf_token() }}",
                "key":key,
                "code":"{{ $code }}",
