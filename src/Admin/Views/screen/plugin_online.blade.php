...
@section('main')
        ...
        <div class="card card-primary card-outline card-outline-tabs">
          <div class="card-header p-0 border-bottom-0">
            <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
              <li class="nav-item">
                <a class="nav-link" href="{{ bc_route_admin('admin_plugin', ['code' => strtolower($code)]) }}" >{{ trans('plugin.local') }}</a>
              </li>
              <li class="nav-item">
                <a class="nav-link active" href="#" >{{ trans('plugin.online') }}</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" target=_new  href="{{ bc_route_admin('admin_plugin.import') }}" ><span><i class="fas fa-save"></i> {{ trans('plugin.import_data', ['data' => 'plugin']) }}</span></a>
              </li>
  ...
  @php
    $scVersion = explode(',', $plugin['BlackCart_version']);
    $scRenderVersion = implode(' ',array_map(
      function($version){
      return '<span title="BlackCart version '.$version.'" class="badge badge-primary">'.$version.'</span>';
      },$scVersion)
    );
    ...
    } elseif(!in_array(config('black-cart.core'), $scVersion)) {
      $pluginAction = '';
    } else {
...
@endphp
                      <tr>
                        <td>{!! bc_image_render($plugin['image'],'50px', '', $plugin['name']) !!}</td>
                        <td>{{ $plugin['key'] }}</td>
            ...
            <div class="block-pagination clearfix m-10">
              ...
              <ul class="pagination pagination-sm no-margin pull-right">
                <!-- Previous Page Link -->
                    ...
                    <li class="page-item"><a class="page-link pjax-container" href="{{ bc_route_admin('admin_plugin_online', ['code' => strtolower($code)]) }}?page={{ $dataApi['current_page'] - 1}}" rel="prev">«</a></li>
                    @endif
                    @for ($i = 1; $i < $dataApi['last_page']; $i++)
                        ...
                        @else
                        <li class="page-item"><a class="page-link" href="{{ bc_route_admin('admin_plugin_online', ['code' => strtolower($code)]) }}?page={{ $i }}">{{ $i }}</a></li>
                        @endif
                    @endfor
                    @if ($dataApi['current_page'] < $dataApi['last_page'])
                    <li class="page-item"><a class="page-link pjax-container" href="{{ bc_route_admin('admin_plugin_online', ['code' => strtolower($code)]) }}?page={{ $dataApi['current_page'] + 1}}" rel="next">»</a></li>
...
<script type="text/javascript">
  function installPlugin(obj,key, path) {
      ...
      $.ajax({
        type: 'POST',
        dataType:'json',
        url: '{{ bc_route_admin('admin_plugin_online.install') }}',
        data: {
          "_token": "{{ csrf_token() }}",
          "key":key,
          "path":path,
