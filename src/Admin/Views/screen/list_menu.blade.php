            ...
            <li class="dd-item " data-id="{{ $level0->id }}">
                <div class="dd-handle header-fix  {{ ($level0->id == $id)? 'active-item' : '' }}">
                  ...
                  <span class="float-right dd-nodrag">
                      <a href="{{ bc_route_admin('admin_menu.edit',['id'=>$level0->id]) }}"><i class="fa fa-edit"></i></a>
                      &nbsp; 
          ...
          @elseif($level0->uri)
            <li class="dd-item" data-id="{{ $level0->id }}">
                <div class="dd-handle {{ ($level0->id == $id)? 'active-item' : '' }}">
                  <i class="{{ $level0->icon }}"></i> {!! bc_language_render($level0->title) !!}
                  <span class="float-right dd-nodrag">
                      <a href="{{ bc_route_admin('admin_menu.edit',['id'=>$level0->id]) }}"><i class="fa fa-edit fa-edit"></i></a>
                      &nbsp; 
                  ...
                  @if($level1->uri)
                    <li class="dd-item" data-id="{{ $level1->id }}">
                        <div class="dd-handle {{ ($level1->id == $id)? 'active-item' : '' }}">
                          <i class="{{ $level1->icon }}"></i> {!! bc_language_render($level1->title) !!}
                          <span class="float-right dd-nodrag">
                              <a href="{{ bc_route_admin('admin_menu.edit',['id'=>$level1->id]) }}"><i class="fa fa-edit fa-edit"></i></a>
                              &nbsp; 
                              ...
                              @if($level2->uri)
                                <li class="dd-item" data-id="{{ $level2->id }}">
                                    <div class="dd-handle {{ ($level2->id == $id)? 'active-item' : '' }}">
                                      <i class="{{ $level2->icon }}"></i> {!! bc_language_render($level2->title) !!}
                                      <span class="float-right dd-nodrag">
                                          <a href="{{ bc_route_admin('admin_menu.edit',['id'=>$level2->id]) }}"><i class="fa fa-edit fa-edit"></i></a>
                                          &nbsp; 
              ...
              <div class="card-header with-border">
                ...
                <div class="card-tools">
                    <div class="btn-group float-right" style="margin-right: 5px">
                        <a href="{{ bc_route_admin('admin_menu.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{trans('admin.back_list')}}</span></a>
                    </div>
                </div>
                @endif
              </div>
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      ...
                      window.location.replace('{{ bc_route_admin('admin_menu.index') }}');
                    }
...
$('.menu-sort-save').click(function () {
    ...
    $.ajax({
      url: '{{ bc_route_admin('admin_menu.update_sort') }}',
      type: 'POST',
      dataType: 'json',
      data: {
        _token: '{{ csrf_token() }}',
