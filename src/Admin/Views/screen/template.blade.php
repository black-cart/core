        ...
        <div class="card card-primary card-outline card-outline-tabs">
          <div class="card-header p-0 border-bottom-0">
            <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
              ...
              <li class="nav-item">
                <a class="nav-link" href="{{ bc_route_admin('admin_template_online.index') }}" >{{ trans('template.online') }}</a>
              </li>
              @endif
              <li class="nav-item">
              ...
              <table class="table table-hover text-nowrap table-bordered">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    @endphp
                   <td>{!!bc_image_render(asset('templates/'.$key.'/'.$pathImage)??'','50px','', $template['config']['name']??'')!!}</td>
        ...
        Swal.fire({
          ...
          if (result.value) {
              ...
              $.ajax({
                type: 'POST',
                dataType:'json',
                url: '{{ bc_route_admin('admin_template.remove') }}',
                data: {
                  "_token": "{{ csrf_token() }}",
                  "key":key,
                },
