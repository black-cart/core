        ...
        <div class="card">
          <div class="card-header">
            ...
            <div class="btn-group float-right" style="margin-right: 5px">
                <a href="{{ bc_route_admin('admin_api_connection.index') }}" class="btn btn-flat btn-default" title="List"><i class="fa fa-list"></i>
          ...
          <div class="card-body p-0">   
            <div class="row">
                <div class="col-md-12 m-2">
                  <input class="switch-data-config" name="api_connection_required" type="checkbox"  {{ (bc_config_global('api_connection_required')?'checked':'') }}><br> {!! trans('api_connection.api_connection_required_help') !!}
                </div>
            </div>
        </div>
        </div>
...
$(document).ready(function() {
    $('#refreshkey').click(function(){
        ...
        $.ajax({
            method: 'get',
            url: '{{ bc_route_admin('admin_api_connection.generate_key') }}',
...
$("input.switch-data-config").bootstrapSwitch();
  $('input.switch-data-config').on('switchChange.bootstrapSwitch', function (event, state) {
      ...
      $.ajax({
        type: 'POST',
        dataType:'json',
        url: "{{ bc_route_admin('admin_config_global.update') }}",
        data: {
          "_token": "{{ csrf_token() }}",
          "name": $(this).attr('name'),
          "value": valueSet
