...
<div class="col-md-12">
  <div class="card" >
    ...
    <div class="card-body" >
      <form action="{{bc_route_admin('admin_banner.index')}}" id="button_search">
        <div class="row justify-content-between">
            ...
            <div class="col-sm-7 d-flex justify-content-end">
                <div class="form-group d-flex">
                  <input type="text" name="keyword" class="form-control mr-3" value="{{$keyword}}" placeholder="{{ trans('banner.admin.search_place') }}">
        ...
        <div class="tools float-right">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end">
                    <a class="dropdown-item" id="button_create_new" href="{{bc_route_admin('admin_banner.create')}}">{{trans('admin.add_new')}}</a>
                    <a class="dropdown-item" id="button_create_new_type" href="{{bc_route_admin('admin_banner_type.index')}}">{{trans('admin.add_new_type')}}</a>
            ...
            <thead>
              <tr>
                ...
                @endif
                  ...
                  <th class="text-center">{{trans('banner.url')}}</th>
                  <th class="text-center">{{trans('banner.sort')}}</th>
                  <th class="text-center">{{trans('banner.status')}}</th>
                  <th class="text-center">{{trans('banner.click')}}</th>
                  <th class="text-center">{{trans('banner.target')}}</th>
                  <th class="text-center">{{trans('banner.type')}}</th>
                  <th>{{trans('order.admin.action')}}</th>
               </tr>
            </thead>
            <tbody>
              @foreach ($dataBann as $keyRow => $row)
              <tr>
                  @if (!empty($removeList))
                  ...
                  <td class="text-center">{{ number_format($row['click']) }}</td>
                  <td class="text-center">{{ $row['target'] }}</td>
                  <td class="text-center">{{ $dataType[$row['type']]??'N/A' }}</td>
                  <td>
                    @include($templatePathAdmin.'Component.action_list',['url_edit'=> bc_route_admin('admin_banner.edit',['id'=>$row['id']]),'id'=>$row['id']])
                  </td>
              </tr>
              @endforeach
            </tbody>
