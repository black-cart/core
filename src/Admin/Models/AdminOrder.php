...
class AdminOrder extends ShopOrder
{
    ...
    public static function getOrderListAdmin(array $dataSearch) {
        ...
        if ($phone) {
            $orderList = $orderList->where(function ($sql) use($phone){
                $sql->Where('phone', 'like' , '%'.$phone.'%' );
            });
        }

        if ($name) {
            $orderList = $orderList->where(function ($sql) use($name){
    ...
    */
    ...
    public static function getSumOrderTotalInYear() {
            return self::selectRaw('DATE_FORMAT(created_at, "%Y-%m") AS ym, SUM(total/exchange_rate) AS total_amount, count(id) AS total_order')
     ...
     * @return  [type]  [return description]
     */
    public static function getSumOrderTotalInWeek() {
        return self::selectRaw('DATE_FORMAT(created_at, "%m-%d") AS d,
        SUM(total/exchange_rate) AS total_amount, count(id) AS total_order')
            ->whereRaw('created_at >=  DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 1 WEEK), "%Y-%m-%d")')
            ->groupBy('d')->get();
    }
