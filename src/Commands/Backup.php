...
class Backup extends Command
{
     ...
     * @var string
     */
    protected $signature = 'bc:backup {--path=} {--includeTables=} {--excludeTables=}';
     ...
     * @var string
     */
    protected $description = 'Backup database "bc:backup --path=abc.sql --includeTables=table1,table2 --excludeTables=t1,t2"';
    const LIMIT = 10;
    ...
    public function handle()
    {
        ...
        try {
            $databaseName = config('database.connections.'.BC_CONNECTION.'.database');
            $userName = config('database.connections.'.BC_CONNECTION.'.username');
            $password = config('database.connections.'.BC_CONNECTION.'.password');
            $host = config('database.connections.'.BC_CONNECTION.'.host');
            $pathMysqlBin = config('database.connections.'.BC_CONNECTION.'.path_mysql_bin');// C:\xampp\mysql\bin
            ...
            bc_report($exception->getMessage());
