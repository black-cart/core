...
class MakePlugin extends Command
{
     ...
     * @var string
     */
    protected $signature = 'bc:make {function} {--name=} {--download=0}';
    ...
    public function plugin($code = 'Other', $key = '', $download = 0) {
        ...
        $pluginKey = bc_word_format_class($key);
        $pluginCode = bc_word_format_class($code);
        if(!in_array($pluginCode, $arrcodePlugin)) {
            $pluginCode = 'Other';
        }
        $pluginUrlKey = bc_word_format_url($key);
        ...
        try {
            ...
            if($download) {
                bc_zip(storage_path($this->tmpFolder."/".$sID), storage_path($this->tmpFolder.'/'.$sID.'.zip'));
                $path = $sID;
            } else {
